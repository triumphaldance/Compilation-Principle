    ...
    def parse(self, input_string: str):
        ...
        while True:
            ...
            if action.startswith('s'):
                ...
                print(f"{step:<5} {stack_content:<20} {input_remaining:<10} s{next_state}")
                stack.append(token)
                ...
                for symble in reversed(production):
                    stack.pop()
                    stack.pop()
                print(f"{step:<5} {stack_content:<20} {input_remaining:<10} r{reduced}")
                state = stack[-1]
                token = key
                ...
                print(f"{step:<5} {stack_content:<20} {input_remaining:<10} 分析成功!")
                break
